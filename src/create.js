...
const showDepartureAirports = (uid) => {
  ...
  return {
    msg: LITERALS.DEPARTURE_AIRPORT[getLang(uid)],
    opts: getOptsKeyboard(buttons),
  };
};

const showDestinationAirports = async (uid) => {
  const selectedAirport = await getArrivalAirports(udata[uid].departure);
  const codes = selectedAirport.map((airport) => airport.arrivalAirport.code);
  const uniqCodes = [...new Set(codes)];

  const buttons = [];
  let row = [];
  uniqCodes.forEach((ruta, i) => {
    if (i > 0 && i % 3 == 0) {
      buttons.push(row);
      row = [];
    }
    row.push({
      text: airportCodes[ruta] || ruta,
  ...
  buttons.push([
    {
      text: LITERALS.ANYWHERE[getLang(uid)],
      callback_data: `action=${ACTIONS.CREATE}&destination=all&next=year1`,
    },
  ]);
  buttons.push([getCancelButton(uid)]);

  return {
    msg: LITERALS.DESTINATION_AIRPORT[getLang(uid)],
    opts: getOptsKeyboard(buttons),
  };
};
...
const chooseYear1 = (uid) => {
  ...
  if (uid === myid)
    buttons.push([
      {
        text: "Escapada Sab-Dom",
        callback_data: `action=${ACTIONS.CREATE}&year1=esc&next=budget`,
      },
    ]);
  buttons.push([getCancelButton(uid)]);

  return {
    msg: LITERALS.CHOOSE_YEAR[getLang(uid)],
    opts: getOptsKeyboard(buttons),
  };
};
...
const chooseMonth1 = (uid) => {
  ...
  const split =
    parseInt(udata[uid].year1) == new Date().getFullYear() ? actualMonth : 0;
  ...
  months.slice(split, months.length).map((month, i) => {
    ...
    row.push({
      text: month,
      callback_data: `action=${ACTIONS.CREATE}&month1=${(
        "0" +
  ...
  return {
    msg: LITERALS.CHOOSE_MONTH[getLang(uid)],
    opts: getOptsKeyboard(buttons),
  };
};
...
const chooseDay1 = (uid) => {
  const thisMonthTotalDays = new Date(
    parseInt(udata[uid].year1),
    parseInt(udata[uid].month1),
    0
  ).getDate();
  let i =
    new Date().getMonth() == parseInt(udata[uid].month1) - 1
      ? new Date().getDate()
      : 1;
  ...
  while (i <= thisMonthTotalDays) {
    ...
    row.push({
      text: i,
      callback_data: `action=${ACTIONS.CREATE}&day1=${("0" + i).slice(
        -2
      )}&next=year2`,
    });
    i++;
  ...
  return {
    msg: LITERALS.CHOOSE_DAY[getLang(uid)],
    opts: getOptsKeyboard(buttons),
  };
};
...
const chooseMonth2 = (uid) => {
  ...
  const split =
    parseInt(udata[uid].year1) == parseInt(udata[uid].year2)
      ? parseInt(udata[uid].month1) - 1
      : 0;
  ...
  months.slice(split, months.length).map((month, i) => {
    ...
    row.push({
      text: month,
      callback_data: `action=${ACTIONS.CREATE}&month2=${(
        "0" +
        (months.indexOf(month) + 1)
      ).slice(-2)}&next=day2`,
    });
...
const chooseDay2 = (uid) => {
  const thisMonthTotalDays = new Date(
    parseInt(udata[uid].year2),
    parseInt(udata[uid].month2),
    0
  ).getDate();
  let i =
    parseInt(udata[uid].year1) == parseInt(udata[uid].year2) &&
    parseInt(udata[uid].month1) == parseInt(udata[uid].month2)
      ? parseInt(udata[uid].day1)
      : 1;
  ...
  while (i <= thisMonthTotalDays) {
    ...
    row.push({
      text: i,
      callback_data: `action=${ACTIONS.CREATE}&day2=${("0" + i).slice(
        -2
      )}&next=duration`,
    });
    i++;
...
const chooseDuration = (uid) => {
  ...
  return {
    msg: LITERALS.CHOOSE_DURATION[getLang(uid)],
    opts: getOptsKeyboard(buttons),
  };
};
...
const chooseBudget = (uid) => {
  ...
  return {
    msg: LITERALS.CHOOSE_BUDGET[getLang(uid)],
    opts: getOptsKeyboard(buttons),
  };
};
...
const insertData = (uid) => {
  ...
  const destination =
    udatos.destination && udatos.destination !== "all"
      ? udatos.destination
      : null;
  if (destination) {
    flight = {
